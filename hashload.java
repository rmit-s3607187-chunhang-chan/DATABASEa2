...
public class hashload implements dbimpl
{
   ...
   public void printRecord(byte[] rec, String input)
   {
      ...
      try{
         ...
         int  bucket = (BN_NAME.hashCode() & 0xfffffff) % 4000000;         // creates a hash and keeps it between 0 - 4m
         byte  BUCKET_SIZE = (byte) 300;                    // size of static bucket
         int  offset = bucket * BUCKET_SIZE;             // offset.

         raf.seek(offset);                // goes to the offset
         raf.writeBytes(BN_NAME);      // writes the String into the particular spot.
